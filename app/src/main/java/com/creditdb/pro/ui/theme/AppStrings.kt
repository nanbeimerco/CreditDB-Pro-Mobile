...
object AppStrings {
    ...
    // --- Tier表機能 ---
    fun tierScreenTitle(isEn: Boolean) = if (isEn) "Tier List" else "Tier表"
    fun tierScreenSubtitle(isEn: Boolean) =
        if (isEn) "Rank your anime, analyze creator taste, and diagnose aesthetic correlation."
        else "作品を自由にドラッグ配置。スタッフ好みの集計や数理指標との相関診断を実行。"

    fun tierAddAnime(isEn: Boolean) = if (isEn) "Add Anime" else "作品追加"
    fun tierStaffAnalysis(isEn: Boolean) = if (isEn) "Staff Affinity" else "スタッフ分析"
    fun tierCorrelationAnalysis(isEn: Boolean) = if (isEn) "Taste Diagnosis" else "相関・診断"
    fun tierEditRows(isEn: Boolean) = if (isEn) "Edit Tiers" else "Tier編集"
    fun tierExport(isEn: Boolean) = if (isEn) "Export / Share" else "出力・保存"

    fun tierSearchPlaceholder(isEn: Boolean) =
        if (isEn) "Search anime to add to Tier list..." else "追加する作品を検索（ローマ字・日本語・英語対応）..."
    fun tierSearchEmpty(isEn: Boolean) = if (isEn) "No anime found" else "該当する作品が見つかりません"
    fun tierAddToTier(isEn: Boolean, tierName: String) =
        if (isEn) "Add to $tierName" else "$tierName に追加"

    fun tierMoveTo(isEn: Boolean) = if (isEn) "Move to Tier..." else "Tierを移動..."
    fun tierRemoveFromList(isEn: Boolean) = if (isEn) "Remove from Tier list" else "Tier表から削除"
    fun tierTapToViewDetails(isEn: Boolean) = if (isEn) "Tap to view details" else "タップで作品詳細を表示"
    fun tierEmptyRowHint(isEn: Boolean) = if (isEn) "No anime in this Tier yet" else "このTierに作品はまだありません"

    fun tierResetDefault(isEn: Boolean) = if (isEn) "Reset to Default Tiers" else "デフォルトのTier構成に戻す"
    fun tierClearAll(isEn: Boolean) = if (isEn) "Clear All Anime" else "すべての作品をクリア"
    fun tierAddRow(isEn: Boolean) = if (isEn) "Add Tier Row" else "新規Tier行を追加"
    fun tierDeleteRow(isEn: Boolean) = if (isEn) "Delete This Tier" else "このTier行を削除"
    fun tierRowName(isEn: Boolean) = if (isEn) "Tier Name" else "Tier名"
    fun tierRowColor(isEn: Boolean) = if (isEn) "Color Accent" else "ラベルカラー"
    fun tierMoveUp(isEn: Boolean) = if (isEn) "Move Up" else "上に移動"
    fun tierMoveDown(isEn: Boolean) = if (isEn) "Move Down" else "下に移動"

    fun tierExportImage(isEn: Boolean) = if (isEn) "Save High-Res Image (PNG)" else "高画質画像として保存 (PNG)"
    fun tierExportImageDesc(isEn: Boolean) = if (isEn) "Save the entire Tier board to gallery." else "現在のTier表全体を画像として端末に保存します。"
    fun tierExportMarkdownCopy(isEn: Boolean) = if (isEn) "Copy AI Analysis Markdown" else "AI分析用Markdownをコピー"
    fun tierExportMarkdownCopyDesc(isEn: Boolean) =
        if (isEn) "Ready to paste into ChatGPT, Claude, or Gemini with mathematical definitions."
        else "偏差値等の数理定義付き。ChatGPT / Claude / Gemini にそのまま貼り付けて深層分析。"
    fun tierExportMarkdownShare(isEn: Boolean) = if (isEn) "Share / Download Markdown (.md)" else "Markdownファイルを共有 / 保存"

    fun tierAffinityTitle(isEn: Boolean) = if (isEn) "Favorite Staff & Studio Analysis" else "スタッフ・制作会社 好み分析"
    fun tierAffinitySubtitle(isEn: Boolean) =
        if (isEn) "Calculated from your Tier placements (weighted by Tier ranking)"
        else "Tier表の配置ランクを重み付けして、あなたの真の好みを部門別・総合で集計"

    fun tierCorrelationTitle(isEn: Boolean) = if (isEn) "Taste & Mathematical Correlation" else "数理指標・感性相関診断"
    fun tierCorrelationSubtitle(isEn: Boolean) =
